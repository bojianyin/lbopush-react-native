...
## 版本历史
##### `V1.2.3` 本地推送正式上线，监听本地推送回调rn，增加了小米透传，获取推送权限是否开启api，并且添加了跳转到app权限设置信息的api，解决了一些已知问题。欢迎issue.
...
```javascript
static sendLocalNotification(String title,String text,Object param):void
发送一个本地推送
```

```javascript
static getisOpenNotification():Promise
获取是否开启推送权限
```

```javascript
static startSettingAppInfo():void
可跳转到一个标准的app权限信息设置页面
```
